...
- `cli/src/pricing.rs` — discount/price conversion, integer-only, no floats: `parse_discount_pct` (decimal-string → basis points), `PRICE_DIMENSIONS` (the one table of the ten price dimensions), `effective_dimensions` (percentage → absolute per-dimension vector), and the renderers `format_per_mtok_usd` / `effective_rate_summary` / `extra_dimension_lines`
...
- `format_per_mtok_usd` is *not* `format_usd`: a unit price can be far below a tenth of a cent, so it renders to whatever precision the value needs (nine places is always exact for nano-dollars), trimming trailing zeroes back to a minimum of three. **Precision must never come from a tier boundary** — a tiered formatter truncates everything below the width it picked (`1_999_999` → `$0.001`), showing the miner less than they are paid. `format_usd` keeps three places for whole-request costs (the `pricing` competitiveness view).
- The `status` product table sizes every column from its own widest rendered cell (`products::status_table_lines`). A fixed width picked against a `$3.000` example overflows on a cheap model with a full price vector (`$0.000999999 in / $0.000000894 out per Mtok (+8 more)`) and pushes the trailing columns off their headers — assert rendered rows, never the constant.
