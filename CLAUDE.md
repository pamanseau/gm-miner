...
- `declare-product` / `declare-products` resolve `--discount-pct` into the **absolute** per-dimension figures client-side, print them, and confirm before any POST — a percentage is input ergonomics, not the thing the miner commits to. `--yes` (and a non-TTY stdin, via `dependency::confirm`'s default) skips the prompt so scripted declarations keep working. The wire body is still `{provider, model, discount_bp, upstream_model}` because the registry's schema is `additionalProperties: false`; when it accepts an absolute vector, the switch is one field on `ProductDeclarationRequest` plus the already-resolved `effective_dimensions` output at the two call sites in `commands/products.rs`.
...
- `format_per_mtok_usd` is *not* `format_usd`: a unit price can be far below a tenth of a cent, so it widens to six or nine decimals rather than rounding a real price to `$0.000`. `format_usd` keeps three places for whole-request costs (the `pricing` competitiveness view).
